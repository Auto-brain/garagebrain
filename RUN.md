...
> ⚠️ `OPENROUTER_API_KEY` нужен для работы чата. Без него сервисы **стартуют
> нормально** (ключ читается без проверки), но `/api/chat` при первом же запросе
> вернёт **500 `ai error`** (OpenRouter отклонит запрос без ключа). В боте это
> выглядит как «⚠️ Не удалось обработать сообщение». Всё, что не обращается к
> Claude (auth, CRUD авто/записей, fuel, загрузка фото, напоминания), работает и
> без ключа.
...
> ⚠️ `JWT_SECRET` у backend и gateway **обязан совпадать** — шлюз подписывает им
> сервисные токены (HS256), а `middleware.Auth` бэкенда проверяет той же строкой.
> Коды ответа при проблемах:
> - оба секрета заданы, но **разные** → бэкенд отдаёт **401 `invalid token`** на
>   `/api/chat` и `/api/upload` (в боте — «⚠️ Не удалось обработать сообщение»);
> - **пустой** секрет у шлюза → токен вообще не выпускается (та же ошибка в боте);
> - **пустой** секрет у бэкенда → бэкенд отдаёт **500 `server misconfigured`**.
